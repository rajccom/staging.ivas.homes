    ...
    if (in_array($_FILES["file"]["type"], $allowedFileType)) {
        ...
        for ($i = 0; $i <= $sheetCount; $i ++) {
            ...
            if (isset($spreadSheetAry[$i][4])) {
                $city_id = mysqli_real_escape_string($conn, $spreadSheetAry[$i][4]);
            }
            

            if (! empty($title) || ! empty($address) || ! empty($phone) || ! empty($direction) || ! empty($city_id)) {
                $query = "insert into address_details(`title`,`address`,`phone`,`direction`,`city_id`) values(?,?,?,?,?)";
                $paramType = "sssss";
                $paramArray = array(
                    $title,
                    $address,
...
<body>
<h2>Import Dealer for Index Page</h2>
